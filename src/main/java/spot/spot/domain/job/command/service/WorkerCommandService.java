...
public class WorkerCommandService implements WorkerCommandServiceDocs {
    ...
    public void registeringWorker(RegisterWorkerRequest request) {
        ...
        Point location = workerCommandMapper.mapLatLngToPoint(request.lat(), request.lng(),
            geometryFactory);
        Worker worker = workerCommandMapper.dtoToWorker(request, member);
        workerUpdatingCommandDsl.updateLocationById(member.getId(), request.lat(), request.lng(),
            location);
    ...
    public void askingJob2Client(ChangeStatusWorkerRequest request) {
        ...
        Matching matching = Matching.builder().job(job).member(worker)
            .status(MatchingStatus.ATTENDER).build();
        matchingRepository.save(matching);
        Member owner = changeJobStatusCommandDsl.getJobsOwner(job.getId());
        FcmDTO msg = fcmMessageUtil.askingJob2ClientMsg(owner.getNickname(), worker.getNickname(),
            job.getTitle());
        ...
        notificationRepository.save(
            notificationMapper.toNotification(msg, NoticeType.JOB, worker, owner.getId()));
    }
    ...
    public void startJob(ChangeStatusWorkerRequest request) {
        ...
        Job job = changeJobStatusCommandDsl.findJobWithValidation(worker.getId(), request.jobId(),
            MatchingStatus.YES);
        changeJobStatusCommandDsl.updateMatchingStatus(worker.getId(), request.jobId(),
            MatchingStatus.START);
        Member owner = changeJobStatusCommandDsl.getJobsOwner(job.getId());
        FcmDTO msg = fcmMessageUtil.startJob2ClientMsg(owner.getNickname(), worker.getNickname(),
            job.getTitle());
        retryTemplate.execute(context -> {
    ...
    public void yesOrNo2RequestOfClient(YesOrNoClientsRequest request) {
        Member worker = userAccessUtil.getMember();
        Job job = changeJobStatusCommandDsl.findJobWithValidation(worker.getId(), request.jobId(),
            MatchingStatus.REQUEST);
        changeJobStatusCommandDsl.updateMatchingStatus(worker.getId(), request.jobId(),
            request.isYes() ? MatchingStatus.YES : MatchingStatus.NO);
        Member owner = changeJobStatusCommandDsl.getJobsOwner(job.getId());
        FcmDTO msg = request.isYes() ?
            fcmMessageUtil.sayYes2ClientMsg(owner.getNickname(), worker.getNickname(),
                job.getTitle()) :
            fcmMessageUtil.sayNo2ClientMsg(owner.getNickname(), worker.getNickname(),
                job.getTitle());
        retryTemplate.execute(context -> {
    ...
    public void continueJob(ChangeStatusWorkerRequest request) {
        Member worker = userAccessUtil.getMember();
        Matching matching = matchingRepository.findByMemberAndJob_Id(worker, request.jobId())
    ...
    @Transactional
    public JobCertifiationResponse certificateJob(ChangeStatusWorkerRequest request,
        MultipartFile file) {
    ...
    public void finishingJob(ChangeStatusWorkerRequest request) {
        ...
        Matching matching = matchingRepository
            .findByMemberAndJob_Id(worker, request.jobId())
            .orElseThrow(() -> new GlobalException(ErrorCode.MATCHING_NOT_FOUND));
        changeJobStatusCommandDsl.findJobWithValidation(worker.getId(), request.jobId(),
            MatchingStatus.START, MatchingStatus.REJECT);
        changeJobStatusCommandDsl.updateMatchingStatus(worker.getId(), request.jobId(),
            MatchingStatus.FINISH);
        Job job = jobRepository.findById(matching.getJob().getId())
    ...
    public void deleteWorker() {
        ...
        Worker worker = workerRepository.findById(me.getId())
            .orElseThrow(() -> new GlobalException(ErrorCode.WORKER_NOT_FOUND));
